...
import {Table, Modal, Button, DatePicker, Input, Select } from "antd";
...
class Tabledata extends React.Component {
  state = { visible: false, list: [], oneDetails: {}, startdate: "", enddate: "", type: '0',search: ''}
  excel = (e) => {
    // console.log("e");
    post(conf.apiurl() + '/api/data/findAllCountXlsx', { startDate: this.state.startdate, endDate: this.state.enddate, accessType: this.state.type, userName: this.state.search,});
  }
  //时间选择框
  selectDate = (date, dateString) => {//这个取消也会获得监听事件
    ...
    setTimeout(() => {
      fetch(conf.apiurl() + '/api/data/findAllCount', {
        method: 'POST',
        mode: "cors",//处理跨域
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
        body: 'startDate=' + this.state.startdate + '&endDate=' + this.state.enddate + '&accessType=' + this.state.type+'&userName='+this.state.search,
      }).then(res => {
        res.json().then(obj => {
          this.setState({
            list: obj.data,
  ...
  handleChange = (value) => {//这个是只有发生改变了，才会执行的，如果现在是入库 ，你再次点击入库不会触发该事件，另外这个也会有延迟
  ...
  search = (e)=>{
    // console.log('sousuo',e.target.value);
    this.setState({
      search: e.target.value,//出去变化时的输入框的值
    });
    setTimeout(() => {
      fetch(conf.apiurl() + '/api/data/findAllCount', {
        method: 'POST',
  ...
  handleCancel = (e) => {
    ...
    fetch(conf.apiurl() + '/api/data/findAllCount', {}).then(res => {//加载页面的函数
      res.json().then(obj => {
    ...
    return (
      ...
      <div className={styles.bigall}>
        ...
        <div className={styles.input}>
          <Search
            placeholder="操作人员"
            // onSearch={value => console.log(value)}//这个是点击搜索才会触发的事件
            onChange={this.search}//这个是框内数据改变就会触发
            style={{ width: 188 }}
          />
        </div>
                {/* //日期选择框 */}
